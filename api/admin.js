...
function setAuthCookie(res) {
  ...
  res.setHeader('Set-Cookie', `mirao_admin=${token}; HttpOnly; Secure; SameSite=Strict; Path=/api/admin; Max-Age=86400`);
}
...
<style>
  ...
  .error { color:#C44; font-size:0.7rem; letter-spacing:0.1em; margin-bottom:1rem; }
</style>
</head>
<body>
<div class="box">
  ...
  if (req.method === 'POST') {
    ...
    if (!code && password !== undefined) {
      if (password === ADMIN_PASSWORD) {
        ...
        return res.status(401).send(loginFormHTML('パスワードが正しくありません'));
      }
    }
...
async function saveName(code) {
  ...
  try {
    var r = await fetch('/api/admin', {
      ...
      body: JSON.stringify({ code: code, name: inp.value.trim() }),
    });
